...
try:
    vertexai.init(project=GCP_PROJECT_ID, location="us-central1") # あなたのプロジェクトのリージョンに合わせてください
...
def handle_message(event):
    reply_message = f"あなたは「{event.message.text}」と送りましたね。まだ開発中です！"
...
async def test_vertex_ai_connection():
    if not gemini_model:
        raise HTTPException(status_code=500, detail="Vertex AI model not initialized. Check logs for errors.")
    try:
        response = await gemini_model.generate_content_async("こんにちは。自己紹介をしてください。")
...
async def test_Maps_connection():
    if not gmaps:
        raise HTTPException(status_code=500, detail="Google Maps client not initialized. Check logs for errors.")
    try:
        # 東京駅周辺のカフェを検索する例
        query = "東京駅 カフェ"
        ...
        if places_result and places_result.get('results'):
            ...
            return {"status": "no_results", "query": query, "message": "No places found for the query."}
